...
async function cmdSessionList(): Promise<void> {
  ...
  for (const s of data) {
    console.log(
      `${String(s.id).slice(0, 8)}  ${String(s.title ?? '')}  ${String(s.model ?? '')}  ${String(s.agent ?? '')}  ${new Date(Number(s.updatedAt ?? s.createdAt ?? Date.now())).toISOString()}`,
    )
  }
}
...
async function cmdAgentList(): Promise<void> {
  ...
  for (const a of data) {
    console.log(
      `${String(a.name)}  [${String(a.permissions)}]  ${String(a.model ?? '')}  tools:${Array.isArray(a.tools) ? (a.tools as string[]).join(',') : ''}`,
    )
...
async function cmdCommandList(): Promise<void> {
  ...
  for (const c of data) {
    console.log(
      `${String(c.name).padEnd(20)}  ${String(c.description ?? '')}  [${String(c.source ?? '')}]${c.agent ? ` agent:${String(c.agent)}` : ''}`,
    )
  }
}
...
async function cmdToolList(): Promise<void> {
  ...
  for (const t of data) {
    console.log(
      `${String(t.name).padEnd(25)}  [${String(t.category)}]  ${String(t.description ?? '').slice(0, 80)}`,
    )
  }
}
  ...
  if (key) {
    ...
    for (const p of parts) {
      if (cur && typeof cur === 'object' && !Array.isArray(cur))
        cur = (cur as Record<string, JsonValue>)[p] as JsonValue | undefined
      else cur = undefined
    }
...
async function cmdConfigSet(opts: Record<string, string | boolean>): Promise<void> {
  ...
  const args = Object.keys(opts)
    .filter((k) => !['key', 'value'].includes(k))
    .map((k) => String(opts[k])) as string[]
  ...
  if (!key || rawVal === undefined) {
    console.error(
      'config set requires <key> <value> — e.g. mira config set model openrouter/anthropic/claude-sonnet-4',
    )
    process.exit(1)
  }
  let value: JsonValue
...
async function cmdFindingList(opts: Record<string, string | boolean>): Promise<void> {
  ...
  const limit =
    typeof opts.limit === 'string' ? `${status ? '&' : '?'}limit=${String(opts.limit)}` : ''
...
async function cmdWorkspaceList(): Promise<void> {
  try {
    ...
    const data = (await res.json()) as {
      workspaces: Array<{ id: string; path: string; name: string; addedAt: number }>
    }
    ...
    try {
      ...
      if (existsSync(fp)) {
        ...
        if (list.length === 0) console.log('No workspaces')
          ...
          for (const w of list)
            console.log(
              `${(w.id ?? '').slice(0, 8)}  ${w.path}  (${w.name ?? w.path.split('/').pop()})`,
            )
        return
      }
    } catch {}
...
async function cmdWorkspaceRemove(opts: Record<string, string | boolean>): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const parsed = JSON.parse(raw) as
        | { workspaces?: Array<{ id: string; path: string; name: string; addedAt: number }> }
        | Array<{ id: string; path: string; name: string; addedAt: number }>
      const list = Array.isArray(parsed) ? parsed : (parsed.workspaces ?? [])
      const idx = list.findIndex(
        (w) => w.id === id || w.path === id || w.id.startsWith(id) || w.path.endsWith(id),
      )
      if (idx === -1) {
        console.error(`workspace not found: ${id}`)
        process.exit(1)
    ...
    if (res.ok) {
      ...
      const found = data.workspaces.find(
        (w) =>
          w.id === target ||
          w.path === target ||
          w.id.startsWith(target) ||
          w.path.endsWith(target),
      )
      if (found) {
        console.log(`Switched to workspace: ${found.path} (${found.name})`)
        console.log(
          `Set MIRA_WORKSPACE=${found.path} for new sessions (export MIRA_WORKSPACE="${found.path}")`,
        )
        return
      }
  ...
  try {
    ...
    if (existsSync(fp)) {
      ...
      const found = list.find(
        (w) => w.id === target || w.path === target || w.id.startsWith(target),
      )
      if (found) {
        console.log(
          `Switched to workspace: ${found.path} (${found.name ?? found.path.split('/').pop()})`,
        )
        console.log(
          `Set MIRA_WORKSPACE=${found.path} for new sessions (export MIRA_WORKSPACE="${found.path}")`,
        )
...
async function cmdProjectList(): Promise<void> {
  ...
  try {
    ...
    if (list.length === 0) {
      console.log(
        'No projects — add one with: mira workspace add /path/to/repo or mira project init --template ts --path /path/to/repo',
      )
      return
    }
    for (const w of list) {
...
async function cmdProjectInit(opts: Record<string, string | boolean>): Promise<void> {
  ...
  const targetPath =
    String(opts.path ?? opts.p ?? positionalPath() ?? process.cwd()).trim() || process.cwd()
...
async function main(): Promise<void> {
  ...
  if (rawCmd.startsWith('/')) {
    console.log(
      `Slash commands are server-side: use via Web/TUI palette (/new, /help) — CLI maps some as: mira session new, mira skill list, mira command list`,
    )
    return
  }
  ...
  switch (cmd) {
    ...
    case 'session':
      if (sub === 'list' || sub === null) await cmdSessionList()
        ...
        console.error(
          `unknown session subcommand: ${sub ?? ''} — try: list, create, new, prompt, import, export`,
        )
        process.exit(1)
      }
      return
