...
interface BusCapture { type: string; payload?: unknown }

async function testApp(): Promise<{ system: LearningSystem; app: Hono<{ Variables: { requestId: string } }>; busEvents: BusCapture[] }> {
...
describe("H3-E knowledge graph write paths", () => {
  test("store→getGraph contains node", async () => {
    ...
    const entry = await system.knowledge.store({
      tier: "semantic", source: "user", title: "Graph node probe", content: "probe content",
    })
  ...
  test("touch bumps lastAccessedAt/accessCount + publishes touched", async () => {
    ...
    const entry = await system.knowledge.store({
      tier: "semantic", source: "user", title: "Touch target", content: "touch me",
    })
  ...
  test("seed stores user entry → 201 + publishes seeded", async () => {
    ...
    const body = await res.json() as { id: string; source: string; tier: string; metadata: Record<string, JsonValue> }
  ...
  test("seed Zod rejects empty/overlong/bad-tier bodies", async () => {
    const { app } = await testApp()
    const post = (body: unknown): Promise<Response> => Promise.resolve(app.request("/knowledge", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  test("promote e2e: finding resolved + entry metadata.findingId + finding edge", async () => {
    ...
    const anchor = await system.knowledge.store({
      tier: "semantic", source: "user", title: "AuthService cache policy", content: "cache notes",
    })
    const f = await writeFinding(db, {
      title: "AuthService retry storm", severity: "major", evidence: "src/auth.ts:42", source: "agent",
    })
    ...
    const graph = await system.knowledge.getGraph(100)
    expect(graph.edges.some(e => e.kind === "finding" && e.from === f.id && e.to === body.entry.id)).toBe(true)
  ...
  test("promote 404s unknown ids, 409s already-resolved", async () => {
    ...
    const f = await writeFinding(db, { title: "Twice promoted", severity: "minor", source: "agent" })
