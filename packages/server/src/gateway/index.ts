...
interface ChatCompletionRequest {
  ...
  tools?: Array<{
    type: 'function'
    function: { name: string; description: string; parameters: z.ZodTypeAny }
  }>
  tool_choice?: 'auto'
  max_tokens?: number
  temperature?: number
...
async function* trackedStream(
  ...
  for await (const chunk of iter) {
    const u =
      chunk?.type === 'finish' ? chunk.usage : chunk?.type === 'usage-report' ? chunk.usage : null
...
export function createGateway(config: MiraConfig): Gateway {
  ...
  function resolveModel(modelStr: string): { baseURL: string; apiKey: string; modelID: string } {
    ...
    if (!provider) {
      ...
      return {
        baseURL: expandEnv(or.options.baseURL),
        apiKey: expandEnv(or.options.apiKey),
        modelID,
      }
    }
    return {
  ...
  return {
    async stream(opts) {
      ...
      if (apiKey) {
        const fallbackModel = (config as MiraConfig & { fallbackModel?: string }).fallbackModel as
          string | undefined
        ...
        for (const candModel of candidates.length ? candidates : [modelID]) {
          const candResolved =
            candidates.length > 1 && candModel !== modelID
              ? resolveModel(candModel)
              : { baseURL, apiKey, modelID: candModel }
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              const iter = await liveOpenAIStream({
                baseURL: candResolved.baseURL,
                apiKey: candResolved.apiKey,
                modelID: candResolved.modelID,
                opts,
              })
              return trackedStream(iter, (usage) =>
                record(candResolved.modelID, usage.input, usage.output, Date.now() - t0),
              )
            } catch (e) {
              ...
              if (!retryable || attempt === 2) {
                console.warn(
                  `[gateway] live stream failed (${candResolved.modelID} attempt ${attempt + 1}):`,
                  msg,
                )
                providerError = msg
                break
          ...
          if (candModel !== modelID)
            console.warn(
              `[gateway] fallback model ${candModel} failed — surfacing error, not substituting a stub`,
            )
        }
        // A key was configured but every candidate failed: fail loudly rather than fabricate content.
        if (providerError) {
          return trackedStream(errorStream(modelID, providerError), () =>
            record(modelID, 0, 0, Date.now() - t0),
          )
        }
      }

      // No provider key configured at all — the clearly-labeled offline stub keeps the loop runnable.
      const stub = stubStream(modelID, opts)
      return trackedStream(stub, (usage) =>
        record(modelID, usage.input, usage.output, Date.now() - t0),
      )
    },
    ...
    async summarize(messages, smallModel) {
      ...
      if (apiKey) {
        try {
          ...
          console.warn(
            '[gateway] live summarize failed, using extractive fallback:',
            (e as Error).message,
          )
        }
      }
      ...
      if (firstUser)
        lines.push(
          `Original task: ${String(typeof firstUser.content === 'string' ? firstUser.content : JSON.stringify(firstUser.content)).slice(0, 300)}`,
        )
      let toolsUsed = 0
...
async function liveOpenAIStream(ctx: {
  baseURL: string
  apiKey: string
  modelID: string
  opts: StreamOptions
...
/** Emit a single error chunk — the SSE layer forwards it as an `error` event and halts the loop. */
async function* errorStream(modelID: string, reason: string): AsyncIterable<GatewayChunk> {
  yield {
    type: 'error',
    error: `[gateway] live stream failed for ${modelID}: ${reason} — no stub substituted.`,
  }
}
...
async function* stubStream(modelID: string, opts: StreamOptions): AsyncIterable<GatewayChunk> {
  ...
  if (hasTools && /read|file|list|glob|search|write|edit/i.test(userText)) {
    yield {
      type: 'text-delta',
      text: `I'll help with: "${userText.slice(0, 80)}". Let me check the project.\n`,
    }
