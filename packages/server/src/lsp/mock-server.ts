...
function handle(msg: any): void {
  ...
  switch (method) {
    ...
    case "textDocument/definition": {
      const target = params.position.character === 0
    ...
    case "textDocument/hover":
      send({ jsonrpc: "2.0", id, result: { contents: { kind: "markdown", value: `**mock** hover at ${params.position.line}:${params.position.character}` } } })
      break
    default:
      if (id !== undefined) {
