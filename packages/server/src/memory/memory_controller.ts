...
export class MemoryController {
  ...
  private _load_experiences(): EpisodicEntry[] {
    try {
      ...
      for (const line of raw.split('\n')) {
        ...
        if (!t) continue
        try {
          out.push(JSON.parse(t) as EpisodicEntry)
        } catch {
          continue
        }
      }
  ...
  private _tfidfRank(
    ...
    if (scored.every((s) => (s.score ?? 0) === 0))
      return this._keyword_match(queryTokens.join(' '), experiences, limit)
    return scored.slice(0, limit)
  }

  private _keyword_match(
    query: string,
    experiences: EpisodicEntry[],
    limit: number,
  ): EpisodicEntry[] {
    ...
    for (const exp of experiences) {
      const text = [String(exp.task ?? ''), String(exp.action ?? ''), String(exp.outcome ?? '')]
        .join(' ')
        .toLowerCase()
      const overlap = qWords.filter((w) => text.includes(w)).length
      if (overlap > 0)
        matches.push({ ...exp, score: Math.round((overlap / qWords.length) * 1e4) / 1e4 })
    }
    matches.sort(
      (a, b) => (b.score ?? 0) - (a.score ?? 0) || (b.timestamp ?? 0) - (a.timestamp ?? 0),
    )
  ...
  get_relevant_skills(task_query: string): string[] {
    try {
      ...
      if (script) {
        const res = spawnSync('python3', [script, task_query, '--json'], {
          encoding: 'utf-8',
          timeout: 15_000,
        })
        if (res.stdout) {
          try {
            const data = JSON.parse(res.stdout) as {
              recommendations?: Array<{ skills?: string[] }>
            }
            const skills: string[] = []
            if (data.recommendations)
              for (const rec of data.recommendations) if (rec.skills) skills.push(...rec.skills)
            if (skills.length) return [...new Set(skills)]
          } catch {}
        }
        // also try python on Windows
        if (!res.stdout && process.platform === 'win32') {
          const res2 = spawnSync('python', [script, task_query, '--json'], {
            encoding: 'utf-8',
            timeout: 15_000,
          })
          if (res2.stdout) {
            try {
              const data = JSON.parse(res2.stdout) as {
                recommendations?: Array<{ skills?: string[] }>
              }
              const skills: string[] = []
              if (data.recommendations)
    ...
    for (const [keywords, skills] of domainSkills)
      if (keywords.some((k) => q.includes(k))) return skills
    return []
  }

  /** Alias */
  getRelevantSkills(query: string): string[] {
    return this.get_relevant_skills(query)
  }
...
export function getMemoryController(opts?: { memoryDir?: string }): MemoryController {
  if (!_shared || opts?.memoryDir)
    return opts?.memoryDir ? new MemoryController(opts) : (_shared ??= new MemoryController())
  return _shared
}
export function sharedMemoryController(): MemoryController {
  return getMemoryController()
}
...
export async function generate_cognitive_packet(
  query: string,
): Promise<{ episodic: unknown[]; semantic: unknown[]; procedural: unknown[]; timestamp: number }> {
  try {
    ...
    const [episodic, semantic, procedural] = await Promise.all([
      kb.retrieve({ query, tier: 'episodic', limit: 3 }).catch((): MemoryEntry[] => []),
      kb.retrieve({ query, tier: 'semantic', limit: 3 }).catch((): MemoryEntry[] => []),
      kb.retrieve({ query, tier: 'procedural', limit: 3 }).catch((): MemoryEntry[] => []),
    ])
