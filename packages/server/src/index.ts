...
async function initOtel() {
  ...
  try {
    ...
    type OtelResource = {
      ...
      getRawAttributes: () => Record<string, string>
    }
    const maybeResource = (
      resources as object as Record<string, { new (attrs: Record<string, string>): OtelResource }>
    )['Resource']
    const fallbackResource = (
      resources as object as {
        default: Record<string, { new (attrs: Record<string, string>): OtelResource }>
      }
    ).default?.['Resource']
...
async function main() {
  ...
  if (process.env.OTEL_EXPORTER_OTLP_ENDPOINT) {
    try {
      ...
      cachedTracer = trace.getTracer('mira-server') as {
        startSpan: (
          name: string,
          opts?: JsonValue,
        ) => { setAttribute: (k: string, v: JsonValue) => void; end: () => void }
  ...
  // Uses Content-Length when present; for chunked bodies (no Content-Length) the
  // raw stream is bounded so the limit can't be bypassed via chunked encoding.
  ...
  app.use('*', async (c, next) => {
    const raw = c.req.raw
    const declared = Number(raw.headers.get('content-length') ?? NaN)
    if (Number.isFinite(declared) && declared > MAX_BODY_BYTES) {
      return c.json({ error: 'payload too large', limit: MAX_BODY_BYTES }, 413)
    }
    // Chunked (no Content-Length) body — enforce the cap by streaming a clone
    // (Request.clone() tees the stream, so downstream handlers still get the body).
    if (
      !Number.isFinite(declared) &&
      (c.req.method === 'POST' || c.req.method === 'PUT' || c.req.method === 'PATCH')
    ) {
      if (await bodyExceedsLimit(raw.clone().body, MAX_BODY_BYTES)) {
        return c.json({ error: 'payload too large', limit: MAX_BODY_BYTES }, 413)
      }
    }
    await next()
  ...
  const mcpCreateSchema = z
    ...
    .superRefine((v, ctx) => {
      if (v.type === 'local' && !v.command?.length)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['command'],
          message: 'local type requires command[]',
        })
      if (v.type === 'remote' && !v.url)
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['url'],
          message: 'remote type requires url',
        })
    })
  ...
  const configPatchSchema = z
    ...
    .refine((v) => v.patch && typeof v.patch === 'object' && Object.keys(v.patch).length > 0, {
      message: 'patch must be non-empty object',
      path: ['patch'],
    })
    ...
    try {
      if (c.req.path.includes('/prompt') || c.req.path.includes('/session')) {
        ...
        if (gw.costUSD || gw.inputTokens || gw.outputTokens) {
          usage = {
            cost_usd: Number(gw.costUSD.toFixed(4)),
            tokens_in: gw.inputTokens,
            tokens_out: gw.outputTokens,
          }
        }
      }
    } catch {}
...
<body><div class="card">
  ...
  function ownerOfSession(sessionID: string): Promise<string | null> {
    ...
    if (!p) {
      const lookup: Promise<string | null> = db.query.sessions
  ...
  app.post('/session/import', async (c) => {
    ...
    const owner =
      API_KEY_OWNERS.size > 0
        ? (resolveOwner(bearerOf(c.req.header('Authorization'))) ?? 'default')
        : null
    ...
    sessionOwnerCache.set(created.id, {
  ...
  app.get('/manager', async (c) => {
    const owner =
      API_KEY_OWNERS.size > 0 ? resolveOwner(bearerOf(c.req.header('Authorization'))) : undefined
  ...
  app.post('/session/:id/revert', async (c) => {
    ...
    try {
      ...
      return c.json({
        ...
        files: reverted.filter(Boolean).map((r) => (r as Snapshot).path),
      })
    } catch (e) {
