...
export function mountMcpRoutes(app: Hono<{ Variables: { requestId: string } }>, mcp: MCPManager) {
  ...
  app.get('/mcp/marketplace', (c: Context) => {
    ...
    if (rawLimit !== undefined) {
      ...
      if (!Number.isFinite(n) || n < 1 || n > 20)
        return c.json({ error: 'limit must be 1-20' }, 400)
      limit = n
    }
  ...
  app.post('/mcp/marketplace/:name', async (c: Context) => {
    ...
    try {
      const cfg: MiraConfig['mcp'][string] = entry.config
  ...
  app.patch('/mcp/:name', async (c: Context) => {
    ...
    try {
      ...
      if (existing)
        await saveConfig(
          { mcp: { ...current, [name]: { ...existing, enabled: parsed.data.enabled } } },
          'project',
        )
      return c.json(entry)
    } catch (e) {
