...
export const inferredTaskSchema = z.object({
  id: z
    ...
    .min(1)
    .max(40)
    .regex(/^[a-z0-9-]+$/, 'task id must match /^[a-z0-9-]+$/')
    .describe('Unique task id (lowercase, digits, dashes)'),
  agent: z
    .string()
    .min(1)
    .max(40)
    .optional()
    .describe('Agent key — must exist in the agent catalog'),
  prompt: z
    .string()
    .min(1)
    .max(10000)
    .describe('Self-contained instructions + acceptance criteria for this subagent'),
  dependsOn: z
    .array(z.string().min(1).max(40))
    .optional()
    .describe('IDs that must complete before this task'),
  contextFrom: z
    .array(z.string().min(1).max(40))
    .optional()
    .describe("Subset of dependsOn whose summaries are forwarded into this task's prompt"),
  budgetSteps: z
    .number()
    .int()
    .min(1)
    .max(25)
    .default(10)
    .describe('Step budget for this node (1-25, default 10)'),
...
export const inferDAGRequestSchema = z.object({
  ...
  tasks: z
    .array(inferredTaskSchema)
    .min(1)
    .max(12)
    .describe('Subtasks (max 12, cost blowup guard)'),
  mergeStrategy: mergeStrategySchema
    .default('lead-synthesis')
    .describe('How to combine node summaries'),
})
...
export function buildPlannerPrompt(
  ...
  return [
    ...
    skillNames
      ? `AVAILABLE SKILLS (mention by name in prompts where relevant):\n${skillNames}`
...
export function validateDAG(
  tasks: InferredTask[],
): { ok: true } | { ok: false; error: string; issue: PlannerIssue } {
  ...
  for (const t of tasks) {
    for (const dep of t.dependsOn ?? []) {
      if (!byId.has(dep)) {
        return {
          ok: false,
          error: `task "${t.id}" dependsOn unknown "${dep}"`,
          issue: {
            rule: 'unknown-dep',
            taskID: t.id,
            detail: `dependsOn "${dep}" matches no task id`,
          },
        }
      }
      if (dep === t.id) {
        return {
          ok: false,
          error: `task "${t.id}" self-dependency`,
          issue: {
    ...
    for (const src of t.contextFrom ?? []) {
      if (!(t.dependsOn ?? []).includes(src)) {
        return {
          ...
          issue: {
            rule: 'contextFrom-subset',
            taskID: t.id,
            detail: `contextFrom "${src}" is not in dependsOn`,
          },
        }
      }
    }
  ...
  if (visited !== tasks.length) {
    return {
      ...
      issue: {
        rule: 'cycle',
        detail: 'dependsOn edges form a cycle (no root task or stuck nodes)',
      },
    }
  }
  return { ok: true }
...
export function formatIssues(issues: PlannerIssue[]): string {
  return issues
    .map((i) =>
      i.taskID
        ? `rule "${i.rule}" on task "${i.taskID}": ${i.detail}`
        : `rule "${i.rule}": ${i.detail}`,
    )
    .join('; ')
}
...
export async function planDAG(
  ...
  let raw: unknown
  try {
    raw = JSON.parse(extractPlannerJSON(text))
  } catch {
    return {
        ...
        issues: [
          {
            rule: 'unknown-agent',
            taskID: t.id,
            detail: `agent "${t.agent}" not in the agent catalog`,
          },
        ],
      }
    }
...
function zodToIssues(err: z.ZodError, raw: unknown): PlannerIssue[] {
  const rawTasks = (
    typeof raw === 'object' && raw !== null ? (raw as Record<string, unknown>)['tasks'] : undefined
