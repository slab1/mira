...
async function mkSession(id: string) {
  await db.insert(db.schema.sessions).values({
    id, title: id, createdAt: Date.now(), updatedAt: Date.now(),
  })
}

beforeEach(async () => {
  if (db) try { db.sqlite.close() } catch {}
...
describe("revertLast", () => {
  ...
  test("reverts are per-session and LIFO", async () => {
    await mkSession("s4"); await mkSession("s5")
    const a = join(dir, "lifo-a.txt")
    const b = join(dir, "lifo-b.txt")
    writeFileSync(a, "A1"); writeFileSync(b, "B1")
    snapshotFile(db, { sessionID: "s4", path: a }); writeFileSync(a, "A2")
    snapshotFile(db, { sessionID: "s4", path: b }); writeFileSync(b, "B2")
...
describe("revertToMessage", () => {
  test("rewinds all mutations at/after message boundary", async () => {
    const s = "s6"
    await mkSession(s)
    await db.insert(db.schema.messages).values({ id: "m1", sessionID: s, role: "user", createdAt: 1000 })
    await db.insert(db.schema.messages).values({ id: "m2", sessionID: s, role: "assistant", createdAt: 2000 })

    const p1 = join(dir, "rw-1.txt"); const p2 = join(dir, "rw-2.txt")
    writeFileSync(p1, "one-v1")
    snapshotFile(db, { sessionID: s, messageID: "m1", path: p1 }); writeFileSync(p1, "one-v2")
    snapshotFile(db, { sessionID: s, messageID: "m2", path: p2 }); writeFileSync(p2, "two-created")
