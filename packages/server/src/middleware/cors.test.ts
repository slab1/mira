...
describe('F5 isOriginAllowed vs corsOrigin parity', () => {
  ...
  test('empty list (dev) → only localhost, not any origin', () => {
    ...
    // null/undefined (no Origin header) blocked — parity with HTTP corsOrigin
  ...
  test('parity: isOriginAllowed true iff corsOrigin returns non-empty (incl. null origin)', () => {
    const cases: Array<{ list: string[]; gate?: string; origin: string | null | undefined }> = [
      ...
      // null origin must be rejected on both WS and HTTP (unified secure behavior)
      { list: [], origin: null },
      { list: [], origin: undefined },
      { list: ['https://slab1.github.io'], origin: null },
      { list: ['https://slab1.github.io'], origin: undefined },
    ]
    for (const c of cases) {
      ...
      const cors = makeCorsOrigin(c.list)(c.origin ?? undefined)
