...
export interface AgentTemplate {
  ...
  model?: string
  /**
   * Optional bash allowlist — when present, only commands whose every
   * `&&`/`||`/`;`/newline/`|` segment starts with one of these read-only
   * prefixes may run under this agent. Enforced in session/prompt.ts
   * lane-contract check, on top of the permission layer.
   */
  bashAllowlist?: readonly string[]
}

/**
 * Read-only bash prefixes for the plan agent (Kilo Architect parity:
 * read+bash, no writes). Mirrors the permission layer's level-0 set —
 * enforced at the template level so destructive commands are denied
 * outright rather than merely gated on user approval.
 */
export const PLAN_BASH_ALLOWLIST = [
  'ls',
  'cat',
  'head',
  'tail',
...
function bashSegmentAllowed(segment: string, allowlist: readonly string[]): boolean {
...
/**
 * Template-level bash gate: every `&&` / `||` / `;` / newline / `|` segment
 * of the command must start with an allowlisted read-only prefix.
 * Empty commands are never allowed.
 */
export function isBashCommandAllowed(command: string, allowlist: readonly string[]): boolean {
  if (!command.trim()) return false
  return command
    .split(/&&|\|\||;|\n/)
    .every((chain) => chain.split('|').every((segment) => bashSegmentAllowed(segment, allowlist)))
}
...
export const AGENT_TEMPLATES = {
  ...
  plan: {
    ...
    permissions: 'standard',
    bashAllowlist: PLAN_BASH_ALLOWLIST,
  },
...
function materialize(name: string, def: AgentDefinition): AgentTemplate | null {
  ...
  const posture =
    def.permissions === 'readonly' || def.permissions === 'elevated' ? def.permissions : 'standard'
  const tools = Array.isArray(def.tools)
    ? def.tools.filter((t): t is string => typeof t === 'string' && t.length > 0)
    : []
  const model =
    typeof def.model === 'string' && def.model.trim().length > 0 ? def.model.trim() : undefined
  const bashAllowlist = Array.isArray(def.bashAllowlist)
    ? def.bashAllowlist.filter((t): t is string => typeof t === 'string' && t.length > 0)
    : undefined
  return {
    ...
    permissions: posture,
    ...(model ? { model } : {}),
    ...(bashAllowlist?.length ? { bashAllowlist } : {}),
  }
}
  ...
  for (const [name, def] of Object.entries(custom)) {
    ...
    if (!tpl) {
      if (!warnedInvalidAgents.has(name)) {
        ...
        console.warn(
          `[agents] ignoring invalid custom agent "${name}" (bad name or system prompt <10 chars)`,
        )
      }
      continue
    }
