...
describe('message queue', () => {
  test('queue → list → clear roundtrip + chained turn drains', async () => {
    ...
    expect(await (await fetch(`${BASE}/session/${session.id}/queue`)).json()).toEqual([
      'first queued',
      'second queued',
    ])
    ...
    // Clear rest
    const cleared = await (
      await fetch(`${BASE}/session/${session.id}/queue`, { method: 'DELETE' })
    ).json()
    expect(cleared.cleared).toBe(promptHadError ? 2 : 1)

    // If drained, chained turn persisted its own user+assistant messages
    if (!promptHadError) {
      const msgs = (await (await fetch(`${BASE}/session/${session.id}/message`)).json()) as Array<{
        role?: string
