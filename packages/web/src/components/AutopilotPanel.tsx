  ...
  return (
    <section class="card" aria-labelledby="autopilot-title">
      <header class="card-head">
        ...
        <span class={`pill ${status()?.status === "running" ? "pill-ok" : "pill-muted"}`}>{status()?.status ?? "idle"}</span>
      </header>

      <Show when={loading()}>
        <div class="skeleton" aria-live="polite">Loading autopilot…</div>
      </Show>
      ...
      <Show when={!loading() && !error()}>
        <div class="grid-2">
          <div class="card">
            <h3>Schedule</h3>
            <dl>
              <div><dt>Next run</dt><dd>{fmt(status()?.nextRunAt)}</dd></div>
              <div><dt>Last run</dt><dd>{fmt(status()?.lastRunAt)}</dd></div>
            </dl>
          </div>
          <div class="card">
            ...
            <Show when={delta()}>
              <div>
                <div class="fs-lg">{delta()!.delta >= 0 ? "+" : ""}{delta()!.delta.toFixed(2)}</div>
