...
export function MemoryGraph(props: Props) {
  const [graph, { refetch }] = createResource(() =>
    api.getKnowledgeGraph(100).catch(() => ({ nodes: [], edges: [] }) as KnowledgeGraph),
  )
  ...
  const seedForm = (prefix: string) => (
    ...
    >
      <div style={{ display: 'flex', 'flex-direction': 'column', gap: '4px' }}>
        <label class="settings-label" for={`${prefix}-seed-title`}>
          Title
        </label>
        <input
      ...
      <div style={{ display: 'flex', 'flex-direction': 'column', gap: '4px' }}>
        <label class="settings-label" for={`${prefix}-seed-content`}>
          Content
        </label>
        <textarea
      ...
      <div style={{ display: 'flex', gap: '8px', 'align-items': 'flex-end' }}>
        <div style={{ display: 'flex', 'flex-direction': 'column', gap: '4px', flex: '1' }}>
          <label class="settings-label" for={`${prefix}-seed-tier`}>
            Tier
          </label>
          <select
            id={`${prefix}-seed-tier`}
    ...
    >
      ...
      <div class="memory-canvas-toolbar">
        <div style={{ display: 'flex', 'align-items': 'center', gap: '8px' }}>
          ...
          <Show when={graph.loading}>
            <span class="pill pill-warn" style={{ 'font-size': 'var(--fs-2xs)' }}>
              loading…
            </span>
          </Show>
        ...
        <div class="memory-canvas-legend" aria-hidden="true">
          <span class="memory-legend-item">
            <span class="memory-legend-dot" style={{ background: 'var(--warn)' }} /> episodic
          </span>
          <span class="memory-legend-item">
            <span
              class="memory-legend-rect"
              style={{ background: 'var(--ok-soft)', 'border-color': 'var(--ok-border)' }}
            />{' '}
            semantic
          </span>
          <span class="memory-legend-item">
            <span
              class="memory-legend-diamond"
                          ...
                          >
                            ...
                            <Show when={n.tier === 'episodic'}>
                              ...
                              <text
                                ...
                                fill="var(--fg-faint)"
                              >
                                {fmtTime(n.updatedAt || n.createdAt)}
                              </text>
                              <text
                                x={12}
                                y={4}
                                ...
                                fill={decayed() ? 'var(--fg-faint)' : 'var(--fg)'}
                              >
                                {label()}
                              </text>
                            </Show>
                ...
                {(node) => (
                  ...
                  >
                    <div class="memory-detail-head">
                      ...
                      <div class="memory-detail-title" style={{ 'margin-top': '8px' }}>
                        {node().label}
                      </div>
                            ...
                            style={{
                              ...
                              background:
                                node().status === 'resolved'
                                  ? 'var(--ok-soft)'
                                  : 'var(--warn-soft)',
                              color: node().status === 'resolved' ? 'var(--ok)' : 'var(--warn)',
                              border: `1px solid ${node().status === 'resolved' ? 'var(--ok-border)' : 'var(--warn-border)'}`,
                            }}
                          >
                            {node().status}
                          </span>
