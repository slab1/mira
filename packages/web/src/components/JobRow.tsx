...
export function JobRow(props: Props) {
  ...
  const badgeBg = () => {
    switch (status()) {
      case "running": return "var(--warn-soft)"
      case "completed": return "var(--ok-soft)"
      case "failed": return "var(--danger-soft)"
  ...
  const badgeBorder = () => {
    switch (status()) {
      case "running": return "var(--warn-border)"
      case "completed": return "var(--ok-border)"
      case "failed": return "var(--danger-border)"
    ...
    >
      ...
      <div style={{ flex: "1", "min-width": "0" }}>
        <div style={{ display: "flex", gap: "6px", "align-items": "center", "margin-bottom": "2px", "flex-wrap": "wrap" }}>
          ...
          <span style={{ "font-size": "var(--fs-2xs)", color: "var(--fg-subtle)", "font-family": "var(--font-mono)" }}>
            {formatElapsed(elapsed())}
          </span>
          <Show when={canOpen()}>
            <span style={{ "font-size": "var(--fs-2xs)", color: "var(--accent)", "font-weight": "600" }}>
              child →
            </span>
          </Show>
        </div>
